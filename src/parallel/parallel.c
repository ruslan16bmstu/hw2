...
int *getBoarders(char *str, char *keys, int process_count) {
  ...
  int *boarders =
      mmap(NULL, (process_count + 1) * sizeof(int), PROT_READ | PROT_WRITE,
...
int maxLen(char *str) {
  ...
  for (int i = 0; i < PRC_CNT; ++i) { // 4 - количество процессов
    ...
    if (pid[i] == -1) {
      if (munmap(result, PRC_CNT * sizeof(int)) ||
          ...
          munmap(str, size + 1) || munmap(pid, PRC_CNT * sizeof(pid_t))) {
  ...
  if (munmap(result, PRC_CNT * sizeof(int)) ||
      munmap(boarders, (PRC_CNT + 1) * sizeof(int)) || munmap(str, size + 1)) {
